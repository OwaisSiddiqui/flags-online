import { useEffect, useState, createContext, PropsWithChildren, useMemo, useContext } from "react"
...
const PusherContext = createContext<{ pusherId: Pusher['connection']['socket_id'] | undefined, pusher: Pusher | undefined } | undefined>(undefined)
...
const PusherProvidier = ({ children }:  PropsWithChildren<{}>) => {
    ...
    useEffect(() => {
        setPusher(new Pusher(PUSHER_APP_KEY, {
            cluster: PUSHER_APP_CLUSTER,
            channelAuthorization: {
                endpoint: isProd() ? `/api/trpc/user.pusherUserAuth` : `http://${getEnv(import.meta.env.VITE_DEV_SERVER_HOST, "VITE_DEV_SERVER_HOST")}:${getEnv(import.meta.env.VITE_DEV_SERVER_PORT, "VITE_DEV_SERVER_PORT")}/api/trpc/user.pusherUserAuth`,
                transport: "ajax",
                customHandler: (params, callback) => {
                    const { socketId, channelName } = params
                    pusherUserAuth.mutateAsync({
                        socketId,
                        channelName
                    }).then((value) => {
                        callback(null, value)
